...
void c_lua::initialize()
{
	...
	int i_read = GetPrivateProfileSectionA("autoload", i_buffer, 65536, "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\menu.ini");
...
void c_lua::download_cloud_scripts() {
	...
	for (auto& s : presets) {
		...
		string savepath = crypt_str("C:/gamesense/lua/" + beta2);
...
void c_lua::refresh_scripts()
{
	...
	std::vector<std::filesystem::path> pathes_to_scan = { "C:/gamesense/lua/" };//,"C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas" };
	...
	for (int l = 0; l < 2; l++) {
		...
		for (auto& entry : std::filesystem::directory_iterator(("C:\\gamesense\\lua")))
		{
