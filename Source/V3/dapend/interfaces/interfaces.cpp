...
static T* FindHudElement(const char* name) {
	static auto fn = *reinterpret_cast<DWORD**>(utilities::pattern_scan("client.dll", "B9 ? ? ? ? E8 ? ? ? ? 8B 5D 08") + 0x1);
...
bool interfaces::initialize() {
	//networkStringTableContainer = get_interface<NetworkStringTableContainer, interface_type::index>("engine.dll", "VEngineClientStringTable001");
	...
	engine_sound = get_interface<c_engine_sound, interface_type::index>("engine.dll", "IEngineSoundClient003");
	...
	//restoreEntityToPredictedFrame = reinterpret_cast<decltype(restoreEntityToPredictedFrame)>(utilities::pattern_scan("client.dll", "55 8B EC 8B 4D ? 56 E8 ? ? ? ? 8B 75"));
	//chat = FindHudElement<c_hudchat>("CHudChat");
	//D3DDEVICE_CREATION_PARAMETERS params;
	//directx->GetCreationParameters(&params);
