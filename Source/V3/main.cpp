...
#include <sdk/MultiThread/Multithread.hpp>
...
DWORD WINAPI main(PVOID base)
{
	...
	if (!exists_test0("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\menu.ini")) {
		std::ofstream menusettings("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\menu.ini");
		menusettings.close();
	}
	std::filesystem::create_directories("C:/gamesense/");
	std::filesystem::create_directories("C:/gamesense/cfg/");
	std::filesystem::create_directories("C:/gamesense/lua/");
	DWORD* nFonts;
...
//void __fastcall hooks::hkClampBonesInBBox(player_t* player, void* edx, matrix3x4_t* matrix, int mask)
//{
...
__forceinline void setup_hooks()
{
	...
	hooks::original_oClampBonesInBBox = (DWORD)DetourFunction((byte*)ClampBonesInBBox, (byte*)hooks::hkClampBonesInBBox);
