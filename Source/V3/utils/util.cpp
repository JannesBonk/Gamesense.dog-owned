...
namespace util
{
	...
	void copy_command(CUserCmd* cmd, int tickbase_shift)
	{
		...
		else
		{
			...
			else if (alway && (c_config::get()->b["rage_dt"] && c_config::get()->auto_check(c_config::get()->i["rage_dt_key"], c_config::get()->i["rage_dt_key_style"])))
		...
		do {
			...
			if (command->m_sidemove > 300.f)
				command->m_sidemove = 300.f;
			else if (command->m_sidemove < -300.f)
				command->m_sidemove = -300.f;

			if (command->m_forwardmove > 300.f)
				command->m_forwardmove = 300.f;
			else if (command->m_forwardmove < -300.f)
				command->m_forwardmove = -300.f;
