...
void render::Draw3DFilledCircleTest(const Vector& origin, float radius, Color color)
{
	...
	for (auto rotation = 0.0f; rotation <= M_PI * 2.0f; rotation += step / 18.0f) //-V1034
	{
		...
		if (math::world_to_screen(pos, screenPos))
		{
			...
			//circle_filled(screenPos.x, screenPos.y, 48, 24, Color(255, 255, 255, 255));
			//triangle(Vector2D(screen.x, screen.y), Vector2D(screenPos.x, screenPos.y), Vector2D(prevScreenPos.x, prevScreenPos.y), Color(color.r(), color.g(), color.b(), color.a()));
		}
	}
