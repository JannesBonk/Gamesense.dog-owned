...
void ui::Begin(const char* Name, GuiFlags flags) {
	...
	if (!(Window->Flags & Flags_ChildWindow) && !(Window->Flags & Flags_ChildWindow) && !ui::ChildsAreStable(Window))
		Window->Block = true;
	else
		Window->Block = false;
...
void ui::End() {
	...
	if (!Window->Block && !(Window->Flags & Flags_ChildWindow)) {
		HandleMoving(Window, Rect{ Vec2(0 - Window->Size.x / 2, 0 - Window->Size.y / 2), Vec2(2560, 1440) });
		HandleResize(Window, Rect{ MenuSize, Vec2(2560, 1440) });
	}
}
...
void ui::EndChild() {
	...
	Render::Draw->Text(ChildName.c_str(), window->Pos.x + 13, window->Pos.y + (text_size.y / -2), LEFT, Render::Fonts::Tahombd, false, window->Dragging ? CMenu::get()->MenuColor() : D3DCOLOR_RGBA(205, 205, 205, g.MenuAlpha));
...
void ui::EndListBox() {
	...
	if (window->PrevCursorPos.y > window->Pos.y + window->Size.y && (window->ScrollRatio >= 0.f || (!window->ParentWindow->Dragging || !ChildsAreStable(window->ParentWindow)))) {
		...
		Render::Draw->FilledRect(Vec2(window->Pos.x + window->Size.x - 7, window->Pos.y + 1), Vec2(6, window->Size.y - 2), D3DCOLOR_RGBA(45, 45, 45, g.MenuAlpha));
		Render::Draw->FilledRect(Vec2(window->Pos.x + window->Size.x - 6, window->Pos.y + 3 + offset), Vec2(4, scroll_height - 6), D3DCOLOR_RGBA(65, 65, 65, g.MenuAlpha));

		if (IsInside(window->Pos.x, window->Pos.y, window->Size.x, window->Size.y))
			window->ScrollRatio = std::clamp(window->ScrollRatio - g.MouseWheel * (5.f / ((int)window->PrevCursorPos.y - (int)((int)window->Pos.y + (int)window->Size.y))), 0.f, 1.f);
...
bool ui::ListBoxSelectableSkins(const char* label, bool activated, const Vec2& size_arg) {
	GuiContext& g = *Gui_Ctx;
	GuiWindow* Window = GetCurrentWindow();
	Rect Framebb{ Window->CursorPos - Vec2(0, 0), Vec2(size_arg.x == 0 ? Window->Size.x : size_arg.x, size_arg.y == 0 ? 16 + (5 * CMenu::get()->DPI()) : size_arg.y) };
	Rect Framebb2{ Window->CursorPos - Vec2(0, 0), Vec2(size_arg.x == 0 ? Window->Size.x - 2 : size_arg.x, size_arg.y == 0 ? 17 + (5 * CMenu::get()->DPI()) : size_arg.y) };
	...
	LPD3DXFONT TextFont = (pressed) ? CMenu::get()->GetFontBold() : CMenu::get()->GetFontDPI();
	Vec2 label_size = Render::Draw->GetTextSize(TextFont, label);
	D3DCOLOR TextColor;
	if (pressed) {
		TextColor = CMenu::get()->MenuColor();
...
{
	if (Selectable(label, *p_selected, size_arg))
	{
...
bool ui::CheckboxOrange(const char* label, bool* selected) {
	GuiContext& g = *Gui_Ctx;
	GuiWindow* Window = GetCurrentWindow();
	Vec2 size = Vec2(8, 8);
	Vec2 Offset = Vec2(0, 0);
	...
	Render::Draw->Text(label, DrawPos.x + 23 + Offset.x, DrawPos.y - 3, LEFT, CMenu::get()->GetFontDPI(), false, D3DCOLOR_RGBA(196, 150, 90, g.MenuAlpha));
	return pressed;
}
...
bool ui::CheckboxBold(const char* label, bool* selected) {
	GuiContext& g = *Gui_Ctx;
	GuiWindow* Window = GetCurrentWindow();
	Vec2 size = Vec2(8, 8);
	Vec2 Offset = Vec2(0, 0);
	...
	Render::Draw->Text(label, DrawPos.x + 23 + Offset.x, DrawPos.y - 3, LEFT, CMenu::get()->GetFontBold(), false, D3DCOLOR_RGBA(192, 192, 192, g.MenuAlpha));
	return pressed;
}
...
bool ui::LabelNew(const char* label) {
	GuiContext& g = *Gui_Ctx;
	GuiWindow* Window = GetCurrentWindow();
	Vec2 size = Vec2(8, 8);
	Vec2 Offset = Vec2(0, 0);
	...
	Render::Draw->Text(label, DrawPos.x + 23 + Offset.x, DrawPos.y - 3, LEFT, CMenu::get()->GetFontDPI(), true, D3DCOLOR_RGBA(192, 192, 192, g.MenuAlpha));
...
bool ui::Selectable(const char* label, bool selected, Vec2 size_arg, Vec2 pos) {
	...
	if (hovered) {
		...
		DrawList::AddText(label, pos.x + 9, pos.y + 3, TextColor, CMenu::get()->GetFontBold(), false);
		...
		nigga = false;
		DrawList::AddText(label, pos.x + 9, pos.y + 3, TextColor, CMenu::get()->GetFontDPI(), false);
	}
	else {
		nigga = false;
		DrawList::AddText(label, pos.x + 9, pos.y + 3, TextColor, CMenu::get()->GetFontDPI(), false);
...
bool ui::SliderBehaviorIDA(const char* item_id, Rect bb, T value, T min_value, T max_value, GuiFlags flags) {
	...
	if (KeyDown(VK_LBUTTON) && hovered) {
		current_window->ActiveID[item_id] = true;
	}

	if (current_window->ActiveID[item_id] /* && current_window->ItemActive[item_id] && current_window->ParentWindow->SelectedItem == item_id*/) {
		g.Blocking = true;
...
void ui::SliderDMG(const char* label, T* v, T v_min, T v_max, const char* format, GuiFlags flags, float scale, int remove) {
	...
	{
		Render::Draw->Text((const char*)(int)*v - 100, slider_bb.Min.x + rect_width - value_width.x / 2, slider_bb.Min.y, LEFT, CMenu::get()->GetFontBold(), true, D3DCOLOR_RGBA(255, 255, 255, min(200, g.MenuAlpha)));
	}*/
	else
...
void ui::Slider2(const char* label, T* v, T v_min, T v_max, const char* format, GuiFlags flags, float scale, int remove) {
	...
	if (hoveredinc && KeyPressed(VK_LBUTTON))
		*v = std::clamp(*v = *v + 0.05f, v_min, v_max);
	if (hoveredinc2 && KeyPressed(VK_LBUTTON))
		*v = std::clamp(*v = *v - 0.05f, v_min, v_max);
...
void ui::Slider(const char* label, T* v, T v_min, T v_max, const char* format, GuiFlags flags, float scale, int remove) {
	...
	Render::Draw->FilledRect(slider_bb.Min + Vec2(0, 0), Vec2(rect_width, slider_bb.Max.y), CMenu::get()->MenuColor());
	Render::Draw->Rect(slider_bb.Min, slider_bb.Max + Vec2(0, 1), 1, D3DCOLOR_RGBA(12, 12, 12, g.MenuAlpha));
	Render::Draw->Gradient(slider_bb.Min + Vec2(1, 1), Vec2(rect_width, slider_bb.Max.y), D3DCOLOR_RGBA(0, 0, 0, 0), D3DCOLOR_RGBA(0, 0, 0, std::clamp(120, 0, g.MenuAlpha)), true);
	Render::Draw->Text(formatted_string, slider_bb.Min.x + rect_width - value_width.x / 2, slider_bb.Min.y, LEFT, CMenu::get()->GetFontBold(), true, D3DCOLOR_RGBA(255, 255, 255, min(200, g.MenuAlpha)));
}
...
bool ui::SingelSelectDisabled(const char* label, int* current_item, std::vector<const char*> items) {
	...
	Render::Draw->Text(buf_display, DrawPos.x + 10, DrawPos.y + 4 + Offset.y, LEFT, CMenu::get()->GetFontDPI(), false, D3DCOLOR_RGBA(72, 72, 72, g.MenuAlpha));
	//End();
	SetCurrentWindow(PickerWindow->ParentWindow);
...
bool ui::SingelSelectNoText(const char* label, int* current_item, std::vector<const char*> items) {
	...
	Rect nigga = { Window->CursorPos.x + 42, Window->CursorPos.y + 11 - 14, float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 26 };
	Vec2 Offset = Vec2(0, 0);
	switch (CMenu::get()->DPI()) {
	case 0:
	{
		Offset = Vec2(0, 0);
		Fullbb = { Window->CursorPos.x + 42, Window->CursorPos.y + 11 - 14 + Offset.y, float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 20 };
		nigga = { Window->CursorPos.x + 42, Window->CursorPos.y + 11 - 14, float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 26 + Offset.y };
		break;
	}
	case 1:
	{
		Offset = Vec2(1, 1);
		Fullbb = { Window->CursorPos.x + 43, Window->CursorPos.y + 11 - 14 + Offset.y, float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 25 };
		nigga = { Window->CursorPos.x + 43, Window->CursorPos.y + 11 - 14 , float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 31 + Offset.y };
		break;
	}
	case 2:
	{
		Offset = Vec2(2, 2);
		Fullbb = { Window->CursorPos.x + 44, Window->CursorPos.y + 11 - 14 + Offset.y, float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 30 };
		nigga = { Window->CursorPos.x + 44, Window->CursorPos.y + 11 - 14 , float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 36 + Offset.y };
		break;
	}
	case 3:
	{
		Offset = Vec2(3, 3);
		Fullbb = { Window->CursorPos.x + 45, Window->CursorPos.y + 11 - 14 + Offset.y, float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 35 };
		nigga = { Window->CursorPos.x + 45, Window->CursorPos.y + 11 - 14 , float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 41 + Offset.y };
		break;
	}
	case 4:
	{
		Offset = Vec2(4, 4);
		Fullbb = { Window->CursorPos.x + 46, Window->CursorPos.y + 11 - 14 + Offset.y, float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 40 };
		nigga = { Window->CursorPos.x + 46, Window->CursorPos.y + 11 - 14 , float(std::clamp(Window->Size.x - 102 + 8, 63.f, 200.f) + (49 * CMenu::get()->DPI())) , 46 + Offset.y };
		break;
	}
