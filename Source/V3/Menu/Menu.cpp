...
void CMenu::Initialize()
{
	...
	ui::CreateContext();
	//dasda
	//network::download_todir();
	//network::open_project();
...
void CMenu::Draw() {
	...
	if (this->m_nCurrentTab == 0) {
		...
		ui::CheckboxBold("SSG-08", &cfg->b["kjnadfv"]); // ADDED!
		ui::LegitButton("    a", &this->legitsel, 1, 6, GuiFlags_LegitTab);
		if (cfg->b["kjnadfv"]) {
			this->legitsel = true;
		}
		ui::EndChild();
		//draw_lua_items("rage", "weapon type"); 
		ui::BeginChild("Aimbot", { Vec2(0, 2), Vec2(3, 8) }, Flags_NoResize | Flags_NoMove);
		...
		ui::BeginChild("Other", { Vec2(6, 0), Vec2(3, 10) }, Flags_NoResize | Flags_NoMove);
	...
	if (this->m_nCurrentTab == 1) {
		ui::BeginChild("Anti-aimbot angle", { Vec2(0, 0), Vec2(3, 10) });
			...
			ui::SingelSelect("Pitch", &cfg->i["aa_pitch"], { "Off", "Down", "Minimal", "Up", "Random", "Pitch up"});
	...
	if (this->m_nCurrentTab == 3) {
		ui::BeginChild("Player ESP", { Vec2(0, 0), Vec2(3, 4) });
			//ui::Label("Activation type");
			ui::CheckboxBold("Enable ESP", &cfg->b["asdasd"]); // ADDED!
			...
			ui::ColorPicker("healbarcol", cfg->c["healbarcoll"], Flags_ColorPicker);
			...
			ui::ColorPicker("wepcol2", cfg->c["wepcoll2"], Flags_ColorPicker);
			...
			ui::Checkbox("Visualize aimbot", &cfg->b["visaimbot"]);// here
		...
		ui::BeginChild("Other ESP", { Vec2(6, 0), Vec2(3, 3) });
			...
			ui::MultiSelectOld("Grenades: Timer", &cfg->m["dsfskfk"], { "Text", "Bar" });
			draw_lua_items("visuals", "other esp");
		ui::EndChild();

		ui::BeginChild("Colored Models", { Vec2(0, 6), Vec2(3, 4) });
			...
			if (cfg->b["player"]) {
				ui::SingelSelectNoText("plyr_Xqz", &cfg->i["plyr_xqzzz"], { "Default", "Solid","Pulsating","Metallic", "Glow", "Crystal", "Bubble" });
			}
			...
			if (cfg->b["teammate"]) {
				ui::SingelSelectNoText("teammate_Xqz", &cfg->i["teammate_xqzzz"], { "Default", "Solid","Pulsating","Metallic", "Glow", "Crystal", "Bubble" });
			}
			ui::Checkbox("Local player", &cfg->b["localplayer"]);
			ui::ColorPicker("lpcol", cfg->c["lpcoll"], Flags_ColorPicker);
			//kpl
			if (cfg->b["localplayer"]) {
				ui::SingelSelectNoText("localmaterial", &cfg->i["localmaterial"], { "Default", "Solid","Pulsating","Metallic", "Glow", "Crystal", "Bubble" });
			}
			...
			if (cfg->b["localplayerfake"]) {
				ui::SingelSelectNoText("fakematerial", &cfg->i["nomaterialyet"], { "Default", "Solid","Pulsating","Metallic", "Glow", "Crystal", "Bubble" });
			}
			...
			if (cfg->b["hands"]) {
				ui::SingelSelectNoText("handchams", &cfg->i["handchams"], { "Default", "Solid","Pulsating","Metallic", "Glow", "Crystal", "Bubble" });
			}
			...
			if (cfg->b["wepvm"]) {
				ui::SingelSelectNoText("weaponchams", &cfg->i["weaponchams"], { "Default", "Solid","Pulsating","Metallic", "Glow", "Crystal", "Bubble" });
			}
			...
			//ui::CheckboxOrange("Fake Impulser", &cfg->b["none1"]); // works
			//ui::Checkbox("FLAG fake", &cfg->b["fakeflag"]); // works
			//ui::Checkbox("FLAG occlusion", &cfg->b["oclusionflag"]); // works
			//ui::Checkbox("FLAG exploit", &cfg->b["xexploitflag"]); // works
			//ui::Checkbox("FLAG kit", &cfg->b["flagskit"]); // works
			//ui::InputText("flag location", c_config::get()->flag_name_text, false, Flags_None);
			//if (c_config::get()->flag_name_text) {
			//	c_config::get()->flag_name = c_config::get()->flag_name_text;
			//	ui::Checkbox("Custom Flag", &cfg->b["moveflag"]); // works
			//	ui::ColorPicker("customflagcol", cfg->c["moveflagcol"], Flags_ColorPicker);
			//}
	...
	if (this->m_nCurrentTab == 4) {
		...
		ui::SliderInt("Thirdperson DISTANCE", &cfg->i["thirdperdist"], 1, 135, "%d°"); // works
		...
		ui::Checkbox("Watermark", &cfg->b["watermarkbla"]); // testing
		...
		ui::Checkbox("Discord Rpc", &cfg->b["discord_rpc"]);
	...
	if (this->m_nCurrentTab == 5) {
		...
		ui::Checkbox("Agent changer", &cfg->b["skins_agentchanger"]); // NEW!
		if (cfg->b["skins_agentchanger"])
			(ui::SingelSelectNoText("Agent t", &cfg->i["skin_changer_model_agent"], {
	"None",
	...
	"tm_jumpsuit_varianta",
		...
		ui::ListBoxSelectableConfig("Abyss", false, Vec2(0, 0));
		ui::EndListBox();
	...
	if (this->m_nCurrentTab == 8) {
		...
		GuiWindow* Window = ui::GetCurrentWindow();
		ui::Checkbox("!SHIFTING TICKBASE! Warning", &cfg->b["warningvis"]); // NEW!
		ui::ColorPicker("warningvisiblecol", cfg->c["namecoll2"], Flags_ColorPicker);
		ui::Checkbox("Money Custom", &cfg->b["moneycustom"]); // NEW!
		ui::ColorPicker("Moneycustcolll", cfg->c["moneycust"], Flags_ColorPicker);
		ui::Checkbox("Custom scope test", &cfg->b["customscope"]); // NEW!
		ui::Checkbox("FLAG fake", &cfg->b["fakeflag"]); // NEW!
		ui::Checkbox("FLAG occlusion", &cfg->b["oclusionflag"]); // NEW!
		ui::Checkbox("FLAG exploit", &cfg->b["xexploitflag"]); // NEW!
		ui::Checkbox("FLAG kit", &cfg->b["flagskit"]); // NEW! 
		//ui::Checkbox("Grenades^", &cfg->b["nadename"]); // &cfg->b["nadename"]
		//ui::ColorPicker("grenadecollup", cfg->c["namenadecoll"], Flags_ColorPicker); // color picker
		ui::InputText("flag location", c_config::get()->flag_name_text, false, Flags_None);
		if (c_config::get()->flag_name_text) {
			c_config::get()->flag_name = c_config::get()->flag_name_text;
			ui::Checkbox("Custom Flag", &cfg->b["moveflag"]); // NEW!
			ui::ColorPicker("customflagcol", cfg->c["moveflagcol"], Flags_ColorPicker);
		}
		if (ui::Button("gamesense")) {
			network::Network_link("https://gamesense.pub/"); // fixed
		}
