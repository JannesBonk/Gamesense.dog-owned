...
{
	if ((!c_config::get()->b["rage_enabled"] && !c_config::get()->auto_check(c_config::get()->i["rage_key_enabled"], c_config::get()->i["rage_key_enabled_st"])) || !e->valid(false))
	{
		if (!e->is_alive())
		{
			is_dormant[i] = false;
...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	e->m_vecAbsVelocity() = e->m_vecVelocity() = e->m_vecVelocity();
