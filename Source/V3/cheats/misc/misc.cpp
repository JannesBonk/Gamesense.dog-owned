...
void misc::watermark()
{
	if (!c_config::get()->b["watermarkbla"])
		return;

	auto game = crypt_str("game           | ");
	auto sense = crypt_str("      sense.test");
	...
	render::get().text(fonts[NAME], x + 13, y + 11, Color(160, 200, 50, 255), HFONT_CENTERED_Y, sense);
}
...
////deleted defensive fucks the accuracy
...
bool misc::double_tap(CUserCmd* m_pcmd)
{
	if (c_config::get()->b["rage_dt"] && c_config::get()->auto_check(c_config::get()->i["rage_dt_key"], c_config::get()->i["rage_dt_key_style"])) {
	{
		...
		{
			bool can_recharge = g_ctx.globals.weapon->is_sniper() ? g_ctx.globals.weapon->can_fire(true) : fabs(g_ctx.globals.fixed_tickbase - last_double_tap) > TIME_TO_TICKS(0.07f);

			if (!aim::get().should_stop && fabs(g_ctx.globals.fixed_tickbase - last_double_tap) > can_recharge)
			{
