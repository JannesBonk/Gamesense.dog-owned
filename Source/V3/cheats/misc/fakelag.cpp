...
{
	if (c_config::get()->b["fakelag"] && c_config::get()->auto_check(c_config::get()->i["hs_key"], c_config::get()->i["hs_key_style"]) && !g_ctx.globals.exploits)
	{
	...
	else
	{
		...
		else if (!g_ctx.globals.exploits && c_config::get()->b["fakelag"] && velocity >= 5.0f && g_ctx.globals.slowwalking && g_cfg.antiaim.fakelag_enablers[FAKELAG_SLOW_WALK])
		{
			if (choked < max_choke)
				g_ctx.send_packet = false;
			else
