...
class aim : public singleton <aim>
{
	...
	int ClipRayToHitbox(const Ray_t& ray, mstudiobbox_t* hitbox, const matrix3x4_t& matrix, trace_t& trace)
	{
		...
		if (!fn)
		{
			// Unable to find the function address
			return -1;
		}

		using ClipRayToHitboxFn = int(__fastcall*)(const Ray_t&, mstudiobbox_t*, const matrix3x4_t&, trace_t&);
		auto clip_fn = reinterpret_cast<ClipRayToHitboxFn>(fn);

		return clip_fn(ray, hitbox, matrix, trace);
	}
