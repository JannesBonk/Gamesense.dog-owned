...
int aim::get_minimum_damage(int health) {
	...
	auto minimum_damage = c_config::get()->i["min_hit_dam"];
...
void aim::StartScan() {
	// Check if the targets vector is empty
	if (targets.empty())
		return;

	// Iterate over each target in the targets vector
	for (auto& target : targets) {
		// Create scan_data objects for the last and history records
		scan_data lastScanData;
		scan_data historyScanData;

		// Check if the last record is valid
		if (target.last_record->valid()) {
			// Adjust the player and scan the last record
			target.last_record->adjust_player();
			Scan(target.last_record, lastScanData);
		}

		// Check if the history record is valid
		if (target.history_record->valid()) {
			// Adjust the player and scan the history record
			target.history_record->adjust_player();
			Scan(target.history_record, historyScanData);

			// Check if the history data is valid and better than the last data
			if (historyScanData.valid() && (lastScanData.valid() ? lastScanData.damage < historyScanData.damage : true))
				scanned_targets.emplace_back(scanned_target(target.history_record, historyScanData));
		}

		// Check if the last data is valid
		if (lastScanData.valid())
			scanned_targets.emplace_back(scanned_target(target.last_record, lastScanData));
	}
}
...
//				break;
//
//			if (point.hitbox == HITBOX_CHEST && point.semi_safe)
//				break;
//
...
bool aim::HitboxIntersection(player_t* player, matrix3x4_t* boneMatrix, int hitbox, const Vector& start, const Vector& end) {
	// Create a trace_t object and initialize its members
	trace_t trace;
	trace.fraction = 1.0f;
	trace.startsolid = false;

	// Create a Ray_t object using the start and end vectors
	Ray_t ray(start, end);

	// Retrieve the studio model of the player
	auto studioModel = m_modelinfo()->GetStudioModel(player->GetModel());
	if (!studioModel)
		return false;

	// Retrieve the hitbox set of the player
	auto hitboxSet = studioModel->pHitboxSet(player->m_nHitboxSet());
	if (!hitboxSet)
		return false;

	// Retrieve the specified hitbox from the hitbox set
	auto studioHitbox = hitboxSet->pHitbox(hitbox);
	if (!studioHitbox)
		return false;

	// Check if the ray intersects with the hitbox
	return ClipRayToHitbox(ray, studioHitbox, boneMatrix[studioHitbox->bone], trace);
}
...
void aim::Scan(adjust_data* record, scan_data& data, const Vector& shoot_position) {
	// Check if the weapon and weapon info are valid
	auto weapon = g_ctx.globals.weapon;
	if (!weapon)
		return;
	...
	// Retrieve the hitboxes of the given record
	auto hitboxes = get_hitboxes(record);
	if (hitboxes.empty())
		return;

	// Initialize variables for the best hitbox and damage
	auto best_hitbox = 0;
	auto best_damage = 0;
	auto minimum_damage = get_minimum_damage(record->player->m_iHealth());

	// Retrieve the points for each hitbox
	std::vector<scan_point> points;
	for (auto hitbox : hitboxes) {
		auto current_points = aim::get_points(record, hitbox, g_ctx.globals.framerate > 60);

		for (auto& point : current_points) {
			point.safe = record->bot || IsSafePoint(record, shoot_position, point.point, hitbox);
			points.emplace_back(point);
		}
	}

	// Check if the points vector is empty
	if (points.empty())
		return;

	// Initialize variables for the best scan data and found flag
	bool found = false;
	scan_data best;

	// Iterate over each point in the points vector
	for (auto& point : points) {
		// Retrieve the fire data for the point
		autowall::returninfo_t fire_data = autowall::get().wall_penetration(shoot_position, point.point, record->player);

		// Check if the fire data is valid and its damage is greater than minimum_damage
		if (fire_data.valid && fire_data.damage > minimum_damage) {
			...
			// Update the best scan data if necessary
			if (best.damage < fire_data.damage && fire_data.hitbox != HITBOX_HEAD) {
				best.point = point;
			...
			if (fire_data.hitbox == HITBOX_HEAD && !found) {
				best.point = point;
				best.visible = fire_data.visible;
				best.damage = fire_data.damage;
				best.hitbox = fire_data.hitbox;
			...
			// Update the given data object with the best scan data
...
void aim::fire(CUserCmd* cmd)
{
	...
	//shot->shot_info.hitchance = hitchance_amount; //for cfg currently.
...
int aim::hitchance(const Vector& aim_angle, player_t* e, int hitchance, CUserCmd* cmd)
{
	...
	// Initialize hit count and needed hit count
	int hits = 0;
	int needed_hits = static_cast<int>(128 * (hitchance / 100.f));

	// Get weapon spread and inaccuracy values
	...
	for (int i = 0; i < 128; i++) {
		...
		// Generate random values for inaccuracy and spread calculations
		...
		// Calculate inaccuracy and spread values
		float inaccuracy = a * weapon_innacuracy;
		float spread = c * weapon_spread;

		// Handle revolver special case
		...
		// Calculate viewangles spread vector and view forward vector
		...
		// Normalize view forward vector and calculate end position
		view_forward.NormalizeInPlace();
		view_forward = src + (direction * weapon_data->flRange);

		// Perform trace to check for hit
		trace_t tr;
		Ray_t ray;
		ray.Init(src, view_forward);
		m_trace()->ClipRayToEntity(ray, MASK_SHOT, e, &tr);

		// Increment hit count if entity was hit
		if (tr.hit_entity == e)
			hits++;

		// Return false if needed hits cannot be achieved
		if ((128 - i + hits) < needed_hits)
			return false;
	}

	// Return true if hitchance is greater than or equal to specified value
