...
void c_config::import() {
	...
	if (is_number(std::to_string(c_config::get()->i["_preset"])) == false) {
		...
		std::string config_name = Config_input_text + std::string(".cfg");

		CreateFileA(("c:\\gamesense\\cfg\\" + config_name).c_str(), 0,0,0,0,0,0);
		std::ofstream fs("c:\\gamesense\\cfg\\" + config_name);
		fs << cfg;
		fs.close();
...
void c_config::export_to_clipboard() {
	...
	if (is_number(std::to_string(c_config::get()->i["_preset"])) == false) {
		...
		str = read_file("C:\\gamesense\\cfg\\" + config_path.filename().string());
...
void c_config::save() {
	...
	sprintf(file_path, "C:/gamesense/cfg/%s.cfg", (std::string(Config_input_text)).c_str());
...
void c_config::refresh_configs() {
	...
	for (auto& entry : std::filesystem::directory_iterator(("C:\\gamesense\\cfg"))) {
...
void c_config::load() {
	...
	char file_path[MAX_PATH] = { 0 };
	sprintf(file_path, "C:/gamesense/cfg/%s", (configs.at(i["_preset"]).c_str()));
...
void c_config::load_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		i[buffer] = GetPrivateProfileIntA("k", buffer, 0, "C:/gamesense/cfg/csgo_keys.cfg");
	}
}
...
void c_config::save_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		WritePrivateProfileStringA("k", buffer, value, "C:/gamesense/cfg/csgo_keys.cfg");
	}
}