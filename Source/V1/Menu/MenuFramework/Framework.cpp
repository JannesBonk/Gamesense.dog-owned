...
void ui::EndChild() {
	...
	if (!window->Name.empty())
		text_size = Render::Draw->GetTextSize(CMenu::get()->GetFontBold(), window->Name.c_str());

	Render::Draw->Text(window->Name.c_str(), window->Pos.x + 14, window->Pos.y + (text_size.y / -2) + 1, LEFT, CMenu::get()->GetFontBold(), false, D3DCOLOR_RGBA(15, 15, 15, g.MenuAlpha));
	Render::Draw->Text(window->Name.c_str(), window->Pos.x + 13, window->Pos.y + (text_size.y / -2), LEFT, CMenu::get()->GetFontBold(), false, window->Dragging ? CMenu::get()->MenuColor() : D3DCOLOR_RGBA(205, 205, 205, g.MenuAlpha));
...
bool ui::LegitButton(const char* label, int* selected, int num, int total) {
	GuiContext& g = *Gui_Ctx;
	GuiWindow* window = GetCurrentWindow();
...
bool ui::ListBoxSelectable(const char* label, bool activated, const Vec2& size_arg) {
	GuiContext& g = *Gui_Ctx;
	GuiWindow* Window = GetCurrentWindow();
	...
	Rect Framebb2{ Window->CursorPos - Vec2(0, 0), Vec2(size_arg.x == 0 ? Window->Size.x - 2 : size_arg.x, size_arg.y == 0 ? 15 + (5 * CMenu::get()->DPI()) : size_arg.y) };
	bool hovered = IsInside(Framebb.Min.x, Framebb.Min.y, Framebb.Max.x, Framebb.Max.y);
	...
	LPD3DXFONT TextFont = (activated && Window->SelectedItem == label) ? CMenu::get()->GetFontBold() : CMenu::get()->GetFontDPI();
	Vec2 label_size = Render::Draw->GetTextSize(TextFont, label);
	D3DCOLOR TextColor;
	if (activated) {
		TextColor = CMenu::get()->MenuColor();
	}
	else if (activated && Window->SelectedItem == label) {
		TextColor = CMenu::get()->MenuColor();
	}
	else {
