...
void CMenu::Draw() {
	...
	if (this->m_nCurrentTab == 0) {
		...
		ui::MultiSelectOld("Weapon", &cfg->m["rage_mphitbox"], { "Global", "SCAR", "SSG 08", "AWP", "R8 Revolver", "Pistol", "Zeus" });
		switch (okkk) {
		case 0:
			ui::RageButton("W", &this->legitsel, 2, 6, GuiFlags_LegitTab);

		case 2:
			ui::RageButton("a", &this->legitsel, 5, 6, GuiFlags_LegitTab);
		}
		ui::EndChild();
		draw_lua_items("rage", "weapon type");
		ui::BeginChild("Aimbot", { Vec2(0, 2), Vec2(3, 8) });
		...
		ui::BeginChild("Other", { Vec2(6, 0), Vec2(3, 8) });
	...
	if (this->m_nCurrentTab == 2) {
		...
		ui::LegitButton("G", &this->legitsel, 0, 6);
		ui::LegitButton("P", &this->legitsel, 1, 6);
		ui::LegitButton("W", &this->legitsel, 2, 6);
		ui::LegitButton("d", &this->legitsel, 3, 6);
		ui::LegitButton("f", &this->legitsel, 4, 6);
		ui::LegitButton("a", &this->legitsel, 5, 6);
	...
	if (this->m_nCurrentTab == 4) {
		...
		ui::SliderInt("TEST DISTANCE", &cfg->i["thirdperdist"], 1, 135, "%d°");
	...
	"tm_jumpsuit_varianta",
		...
		draw_lua_items("SkinChanger", "skintab1");
	...
	if (this->m_nCurrentTab == 7) {
		...
		if (oldscriptid != scriptIDD) {
			LI_FN(GetPrivateProfileStringA).get()(crypt_str("autoload"), crypt_str(rand.c_str()), "", szStartupval, 64, crypt_str("C:\\gamesense\\luas\\menu.ini"));
		...
		WritePrivateProfileStringA("autoload", rand.c_str(), to_string(startupshit).c_str(), "C:\\gamesense\\luas\\menu.ini");
