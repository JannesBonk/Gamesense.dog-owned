...
void DrawBeamPaw(Vector src, Vector end, Color color)
{
	...
	beamInfo.m_flRed = c_config::get()->c["grentrajcoll"][0];
	beamInfo.m_flGreen = c_config::get()->c["grentrajcoll"][1];
	beamInfo.m_flBlue = c_config::get()->c["grentrajcoll"][2];
...
bool c_grenade_prediction::data_t::draw(entity_t* entity) const
{
	...
	//static auto size = Vector2D(35.0f, 5.0f);
	render::get().circle_filled(prev_screen.x, prev_screen.y - size.y * 0.5f, 85, 32, Color(0, 0, 0));
	render::get().circle_filled(prev_screen.x, prev_screen.y - size.y * 0.2f, 85, 20, Color(200, 25, 25, alpha_damage));
	render::get().circle_filled(prev_screen.x, prev_screen.y - size.y * 0.2f, 85, 22, Color(200, 25, 25, alpha_damage));
	render::get().circle_filled(prev_screen.x, prev_screen.y - size.y * 0.2f, 85, 23, Color(200, 25, 25, alpha_damage));
	render::get().circle_filled(prev_screen.x, prev_screen.y - size.y * 0.2f, 85, 21, Color(200, 25, 25, alpha_damage));
	draw_arc(prev_screen.x, prev_screen.y - size.y * 0.5f, 33, 0, 360 * percent, 2.5f, Color(255, 255, 255));
	render::get().text(fonts[GRENADES], prev_screen.x + 1.0f, prev_screen.y - size.y * 0.5f - 6.0f, Color(255, 255, 255, 144), HFONT_CENTERED_X | HFONT_CENTERED_Y, index_to_grenade_name_icon(m_index));
	render::get().text(fonts[HE], prev_screen.x, prev_screen.y - size.y * 0.5f + 8.0f, Color(255, 255, 255), HFONT_CENTERED_X, "%i", (int)distance);
	...
	//render::get().circle_filled(position.x, position.y - size.y * 0.5f, 60, 20, Color(c_config::get()->c["grenproxcoll"][0], c_config::get()->c["grenproxcoll"][1], c_config::get()->c["grenproxcoll"][2], c_config::get()->c["grenproxcoll"][3] ));
	//render::get().circle_filled(position.x, position.y - size.y * 0.5f, 60, 20, Color(c_config::get()->c["grenproxcoll2"][0], c_config::get()->c["grenproxcoll2"][1], c_config::get()->c["grenproxcoll2"][2], alpha_damage));
	//draw_arc(position.x, position.y - size.y * 0.5f, 20, 0, 360 * percent, 2, Color(c_config::get()->c["grenproxcoll2"][0], c_config::get()->c["grenproxcoll2"][1], c_config::get()->c["grenproxcoll2"][2], c_config::get()->c["grenproxcoll2"][3]));
