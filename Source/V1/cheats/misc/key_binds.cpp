...
void key_binds::update_key_bind(key_bind* key_bind, int key_bind_id)
{
	...
	case HOLD:
		...
		case 2:
			...
			if (misc::get().double_tap_key && c_config::get()->b["rage_dt_key"] != c_config::get()->b["hs_key"])
		...
		case 12:
			...
			if (misc::get().hide_shots_key && c_config::get()->b["hs_key"] != c_config::get()->b["rage_dt_key"])
