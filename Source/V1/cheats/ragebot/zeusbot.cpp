...
void zeusbot::run(CUserCmd* cmd)
{
	...
	if (!c_config::get()->b["misc_zeusbot"])
		return;
...
void zeusbot::fire(CUserCmd* cmd)
{
	...
	if (!c_config::get()->b["rage_silent_enabled"])
	...
	if (net_channel_info)
	{
		...
		if (misc::get().double_tap_enabled && c_config::get()->auto_check(c_config::get()->i["rage_key_enabled"], c_config::get()->i["rage_key_enabled_st"]))
