...
bool playeresp::draw_ammobar(player_t* m_entity, const Box& box)
{
	...
	Vector dist = g_ctx.local()->m_vecOrigin() - m_entity->m_vecOrigin();
	float flDist = sqrtf(dist.Length());

	if (flDist < 1.f)
		flDist = 1.f;

	int flDistShow = flDist / 0.254 - 25;
	...
	std::string ft_bar = " FT";

	if (cfgsys->b["distance"])
	{
		render::get().text(fonts[ESP], n_box.x + 15, n_box.y + 8, text_color, HFONT_CENTERED_X | HFONT_CENTERED_Y, std::to_string(flDistShow).c_str(), ft_bar.c_str());
	}
...
void playeresp::draw_name(player_t* m_entity, const Box& box)
{
	...
	if (m_engine()->GetPlayerInfo(m_entity->EntIndex(), &player_info))
	{
		...
		render::get().text(fonts[NAME], box.x + box.w / 2 + 1, box.y - 12, Color(0,0,0,255), HFONT_CENTERED_X, name.c_str());
	...
	if (cfgsys->b["flags"])
	{
		...
		if (animstate)
		{
			...
			if (fakeducking() && e->m_fFlags() & FL_ONGROUND && !animstate->m_bInHitGroundAnimation)
			{
				...
				render::get().text(fonts[ESP], _x, _y, color, HFONT_CENTERED_NONE, "DUCK");
				_y += 8;
			}
		}
	}
