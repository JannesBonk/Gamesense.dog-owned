...
void otheresp::indicators()
{
	...
	Color FPSCOL = percent_col((float(std::clamp(g_ctx.globals.framerate, 30, 200) - 64)) / 200.f * 100.f);
	...
	{
		otheresp::get().m_indicators.push_back({ "OSAA", D3DCOLOR_RGBA(255, 255, 255,255) });
	}
...
void otheresp::create_fonts() {
	...
	IndFont = create_font2("Calibri", 30 * CMenu::get()->GetDPINum(), 600, FONTFLAG_ANTIALIAS);
	IndShadow = create_font2("Calibri", 30 * CMenu::get()->GetDPINum(), 600, FONTFLAG_ANTIALIAS);
}
...
void otheresp::draw_indicators()
{
	...
	for (auto& indicator : m_indicators)
	{
		...
		render::get().gradient(14, height - 340 - h - 3, render::get().text_width(IndShadow, indicator.str.c_str()) / 2, 33, Color(0, 0, 0, 0), Color(0, 0, 0, 165), GRADIENT_HORIZONTAL);
		render::get().gradient(14 + render::get().text_width(IndShadow, indicator.str.c_str()) / 2, height - 340 - h - 3, render::get().text_width(IndShadow, indicator.str.c_str()) / 2, 33, Color(0, 0, 0, 165), Color(0, 0, 0, 0), GRADIENT_HORIZONTAL);
