...
void __fastcall hooks::hooked_runcommand(void* ecx, void* edx, player_t* player, CUserCmd* m_pcmd, IMoveHelper* move_helper)
{
	...
	if (c_config::get()->b["rage_enabled"] && player->is_alive())
	{
		...
		if (weapon)
		{
			...
			if (weapon->m_iItemDefinitionIndex() == WEAPON_REVOLVER)
			{
				...
				auto tickrate = g_ctx.globals.tickrate;
...
bool __stdcall hooks::hooked_inprediction()
{
	...
	if (((c_config::get()->b["rage_enabled"] && c_config::get()->auto_check(c_config::get()->i["rage_key_enabled"], c_config::get()->i["rage_key_enabled_st"])) || g_cfg.legitbot.enabled) && g_ctx.globals.setuping_bones && (uintptr_t)_ReturnAddress() == maintain_sequence_transitions)
		return true;
