...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	else if (!strcmp(event_name, crypt_str("player_death")))
	{
		...
		if (g_ctx.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			if (c_config::get()->auto_check(c_config::get()->i["esp_en"], c_config::get()->i["esp_en_type"]) && g_cfg.esp.kill_effect)
	...
	else if (!strcmp(event_name, crypt_str("round_start")))
	{
		...
		for (auto i = 1; i < m_globals()->m_maxclients; i++)
		{
			...
			if (g_ctx.globals.loaded_script)
				for (auto current : c_lua::get().hooks.getHooks("aim_miss"))
					current.func();
		}
		...
		if (g_ctx.globals.loaded_script)
			for (auto current : c_lua::get().hooks.getHooks("on_round_start"))
				current.func();
	}
	...
	if ((c_config::get()->b["tracers"] || g_cfg.esp.enemy_bullet_tracer) && c_config::get()->auto_check(c_config::get()->i["esp_en"], c_config::get()->i["esp_en_type"]))
