...
{
	setup_item(&g_cfg.esp.grenadetrajectory, Color(255, 50, 1), crypt_str("esp.grenadetrajectory"));
	setup_item(&g_cfg.esp.grenadetrajectory_hit, Color(153, 255, 1), crypt_str("esp.grenadetrajectory_hit"));
	...
	setup_item(&g_cfg.ragebot.fast_recharging, false, crypt_str("Ragebot.fast_recharging"));
	//
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].ignore_unsafe_hitboxes, 9, std::to_string(i) + crypt_str("Ragebot.ignore_unsafe_hitboxes"));
		...
		//
		setup_item(&g_cfg.ragebot.weapon[i].adaptive_shot_dmg, false, std::to_string(i) + crypt_str("Ragebot.adaptive_shot_dmg"));
	...
	for (auto i = 0; i < 4; i++)
	{
		...
		setup_item(&g_cfg.antiaim.type[i].yawjit, 0, std::to_string(i) + crypt_str("Antiaim.yawjit"));//new
		setup_item(&g_cfg.antiaim.type[i].yaw_jit, 0, std::to_string(i) + crypt_str("Antiaim.yaw_jit"));//new
...
void C_ConfigManager::save(std::string config)
{
	...
	auto get_dir = [&folder, &file, &config]() -> void
	{
		...
		{
			folder = std::string(path) + crypt_str("\\hentaiware\\configs\\");
			file = std::string(path) + crypt_str("\\hentaiware\\configs\\") + config;
		}
