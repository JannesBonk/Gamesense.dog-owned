...
namespace ns_render
{
	...
	void draw_rect_filled(float x, float y, float w, float h, int r, int g, int b, int a)
	{
		render::get().rect_filled((int)x, (int)y, (int)w, (int)h, Color(r, g, b, a));
	}
    ...
    // void draw_logmsg(float x, float y, float x2, float y2, float x3, float y3, Color color)
	//{
	//	 auto logs_size_inverted = 10 - logs.size();
	//	 render::get().text(fonts[LOGS], x, y + y - logs_size_inverted * 14, log.color, HFONT_CENTERED_NONE, log.message.c_str());
	//}
}
...
void c_lua::initialize()
{
	...
	client[crypt_str("unix_time")] = ns_client::unix_time;
	...
	//renderer[crypt_str("logmesg")] = ns_render::draw_triangle;
	...
	int i_read = GetPrivateProfileSectionA("autoload", i_buffer, 65536, "C:\\gamesense\\luas\\menu.ini");
...
void c_lua::refresh_scripts()
{
	...
	std::vector<std::filesystem::path> pathes_to_scan = { "C:\\gamesense\\luas\\cloud","C:\\gamesense\\luas" };
	CreateDirectory("C:\\gamesense\\luas\\", NULL);
	CreateDirectory("C:\\gamesense\\luas\\cloud\\", NULL);
	for (int l = 0; l < 2; l++) {
